...
public class LogisticService {
    ...
    public LogisticResponseDTO queryLogisticInfo(LogisticRequestDTO logisticRequestDTO) {
        ...
        TrackingRequest trackingRequest = logisticMapper.toTrackingRequest(logisticRequestDTO);
        ...
        if(!CollectionUtils.isEmpty(list)){
            ...
            cachedLogisticList.add(posLogisticQuery(trackingRequest, hash));
            return getCachedLogisticResponseDTO(cachedLogisticList);
        }

    }

    private CachedLogistic posLogisticQuery(TrackingRequest trackingRequest, String hash) {
        TrackingResponseDTO trackingResponseDTO = trackingService.posLogisticRestCall(trackingRequest);
        CachedLogistic cachedLogistic = new CachedLogistic();
        cachedLogistic.setCourier(TRACKING);
        cachedLogistic.setError(trackingResponseDTO.getError());
        if(trackingResponseDTO.getError() == null){
            cachedLogistic.setRate(trackingResponseDTO.getPrice());
            cachedLogistic.setHash(hash);
            logger.info("saving pos details to database");
